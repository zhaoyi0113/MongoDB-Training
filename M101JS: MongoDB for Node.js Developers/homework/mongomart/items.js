...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        const item = this.db.collection('item');
        item.aggregate([
          {$match: {category: {$exists: true}}},
          {$group: {_id: '$category', num: {$sum:1} }},
          {$sort: {'_id':1}}   ], (err, results) => {
            categories = results;
            item.count((err, c)=>{
              categories.unshift({_id:'All', num: c});
            });
            callback(categories);
          });
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        item.find(condition).skip(itemsPerPage*page)
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        const item = this.db.collection('item');
        item.find({$text: {$search: query}}).skip(page*itemsPerPage).limit(itemsPerPage).sort({_id:1})
          .toArray((err, results)=>{
            callback(results);
          });
        // TODO Include the following line in the appropriate
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        const item = this.db.collection('item');
        item.find({$text: {$search: query}}).count((err, c)=>{
          callback(c);
        });
        // callback(numItems);
    }
    ...
    this.getItem = function(itemId, callback) {
        ...
        const item = this.db.collection('item');
        item.find({_id: itemId}).toArray((err, results)=>{
          callback(results[0]);
        });
        // TODO Include the following line in the appropriate
